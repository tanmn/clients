...
class ViberShell extends AppShell
{
    ...
    protected function process($date = NULL)
    {
        ...
        $raw_groups = $this->Message->Event->ChatInfo->find('all', array(
            'fields' => array(
                'ChatInfo.ChatID',
                'ChatInfo.Name',
                'ChatInfo.Token'
        ...
        foreach ($raw_groups as $group)
        {
            ...
            if (!empty($group['ChatRelation']))
            {
                foreach ($group['ChatRelation'] as $member)
                {
                    $summary[] = array(
                        'report_date' => $date,
                        'hotline' => MY_NUM,
                        ...
                        'number' => $this->formatNumber($member['Number']),
                        'quantity' => 0
                    );
                }
            }
        ...
        {
            if (isset($group_tokens[$item['group_code']]))
            {
    ...
    protected function mailTop250($data = NULL)
    {
        ...
        $message = 'Dear administrators,';
        $message .= "\n\n";
        $message .= 'I send you the list of top 250 winners on ' . $date . ' from ALL hotlines.';
        $message .= "\n\n";
        $message .= "-------LIST BEGIN-------";
        $message .= "\n\n";
        ...
        {
            $index = $key + 1;
            $is_bot = ($value['MasterPoint']['points'] > $this->threshold) ? ' (!!!MAY BE BOT!!!)' : '';
